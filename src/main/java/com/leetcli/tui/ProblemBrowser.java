...
public class ProblemBrowser {
    ...
    public ProblemBrowser(LeetCodeClient client, ConfigManager config, Terminal terminal, PrintWriter writer) {
        this.client = client;
        this.config = config;
    ...
    public void setInitialState(String difficulty, String search, int page) {
        if (difficulty != null) {
    ...
    public void run() throws IOException {
        ...
        try {
            ...
            if (!externalTerminal) {
                ...
                writer.print(Theme.AUTOWRAP_ON);
                writer.flush();
            }
        }
    }
    ...
    private void openSelected() {
        ...
        try {
            ...
            // Pass current terminal and writer to MainTUI
            MainTUI tui = new MainTUI(client, detail, config, terminal, writer);
            tui.run();
            
            // Re-entering browser: reset terminal state slightly
            writer.print(Theme.ALT_BUF_ON + Theme.HIDE_CURSOR + Theme.AUTOWRAP_OFF + Theme.CLEAR);
            writer.flush();
        } catch (Exception e) {
    ...
    private void render() {
        ...
        sb.append(Theme.TITLE_BG).append(padRight(titleBar, W)).append(Theme.RESET);
        ...
        sb.append(cursorTo(row++));
        sb.append(Theme.DIM).append("─".repeat(W)).append(Theme.RESET);

        // ── Problem rows ──
        int listH = H - 4; // Title, Headers, Divider, Footer
        ...
        if (searchMode) {
            ...
            footer = String.format(
                    "  ↑↓ move  Enter open  / search  d diff  n/p page %d/%d  r refresh  q quit%s",
